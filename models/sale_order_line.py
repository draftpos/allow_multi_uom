...
class SaleOrderLine(models.Model):
    ...
    def _get_uom_specific_pricelist_price(self):
        ...
        matching_rule = self.env['product.pricelist.item'].search([
            ...
            '|', ('min_quantity', '=', 0.0),
                 ('min_quantity', '<=', qty),
        ], order='applied_on, min_quantity desc, id desc', limit=1)

        if not matching_rule:
            return False

        if matching_rule.compute_price == 'fixed':
            return matching_rule.fixed_price
